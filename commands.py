...
def horario_brasilia():
    ...
    horario_formatado = horario_atual_brasilia.strftime("%d/%m/%Y às %H:%M:%S")
...
async def prod(ctx):
    ...
    if prod >= 9000:
        ...
        embed.set_image(url=(f"{random.choice(imgProd)}"))
        embed.set_footer(text=(f"Comando realizado em {horario_brasilia()}"))
        ...
        embed.set_author(name=ctx.author.name, icon_url=ctx.author.avatar)
        embed.set_image(url=(f"{random.choice(imgPlayer)}"))
        embed.set_footer(text=(f"Comando realizado em {horario_brasilia()}"))
        await ctx.send(embed=embed)
